...
class Tester:
    def __init__(self, test_dataset, device):
        self.device=device
        self.test_loader=DataLoader(test_dataset, batch_size=64, drop_last=True)
    ...
    def __load_model(self, ckpt_path, h):
        ...
        return model,hp["structure_results"]
            
    def test_model(self,ckpt_path,h,t):
        model,structure_results=self.__load_model(ckpt_path,h)
        metrics=self.evaluator.evaluate_threshold(model,self.test_loader,t)
        print(f"Training model performance,best metrics{structure_results}")
        return metrics
