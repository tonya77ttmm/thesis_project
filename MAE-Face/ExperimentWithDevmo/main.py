...
from .sampling.label01_sampler import LabelO1Sampler
...
def build_dataset(dataset_path, label_path, sampler,parser_class):
    ...
    print("label counts:", np.bincount(label_df['label']))
...
def build_feature_dataset(feature_dir, save_prefix):
    ...
    clip_path = feature_dir / f"{save_prefix}_clip_ids.npy"
    num_samples = os.path.getsize(feature_path) // (768 * 4)
    return FeatureDataset(feature_path, label_path, clip_path, num_samples)
...
def train_mlp(feature_dir, save_prefix, hidden_grid, lr_grid, wd_grid, drop_grid, thresh_grid, num_classes, input_size, num_epochs, device):
    ...
    print("unique clips:",len(np.unique(feature_dataset.clip_ids)))
...
def experiment_with_devmo_sampler_01(devmo_dataset_path,devmo_train_label_path,devmo_test_dataset,mae_extractor,batch_size,feature_dir,hidden_grid, lr_grid, wd_grid, drop_grid, thresh_grid, num_classes, input_size, device):
    sampler_01=LabelO1Sampler(fps=15)
    devmo_train_dataset=build_dataset(devmo_dataset_path,devmo_train_label_path,sampler_01,DevmoParser)
    
    extract_features(mae_extractor,batch_size,devmo_train_dataset,feature_dir,"devmo_train_sampler_01")
    train_mlp(feature_dir, "devmo_train_sampler_01", hidden_grid, lr_grid, wd_grid, drop_grid, thresh_grid, num_classes, input_size, num_epochs=60, device=device)
...
#     devmo_test_label = feature_dir/"devmo_test_labels.npy"
    ...
    #CV train model on devmo_sampler_01 train dataset
    experiment_with_devmo_sampler_01(devmo_dataset_path, devmo_train_label_path, devmo_test_dataset, mae_extractor, batch_size, feature_dir, hidden_grid, lr_grid, wd_grid, drop_grid, thresh_grid, num_classes, input_size, device)
    ...
    for h in hidden_grid:
        ...
        for t in thresh_grid:
            ...
            metrics=tester.test_model(model_dir/f"MLP_best_structure_model_{hidden_str}.pth",h,t)
            print(f"Metrics for hidden layers {h} at threshold {t}: {metrics}")
