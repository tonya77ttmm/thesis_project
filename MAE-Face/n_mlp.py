...
class FeatureDataset(Dataset):
    def __init__(self, feature_dir):
        self.samples = []
        # Assuming you saved each frame as a .pt file 
        # OR you have one big file with all frames.
        # If each file is one frame:
        for feature_file in os.listdir(feature_dir):
            if feature_file.endswith('.pt'):
                self.samples.append(os.path.join(feature_dir, feature_file))
    ...
    def __getitem__(self, idx):
        ...
        # If data['features'] is [1, 768], use .squeeze(0) to get [768]
        features = data['features'].squeeze() 
        label = int(data['labels']) # Ensure it's a single integer
...
def train_mlp_grid_search(input_size,hidden_layers,dropout_rate,num_classes,learning_rate=1e-3, weight_decay=1e-5,num_epochs=200,patience=5,device='cuda'):
    ...
    # ===== DATASETS =====
    train_dataset = FeatureDataset("./Features/Training_single_frame_features")
    val_dataset = FeatureDataset("./Features/Val_single_frame_features")
